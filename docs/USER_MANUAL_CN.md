...
本工具采用离线公私钥授权。上位机仅包含验签公钥，不能自行生成注册码；授权私钥仅由软件供应方离线保管。
...
1. 打开软件，点击【授权管理】。
2. 若使用本机绑定授权，点击【复制机器码】并发送给软件供应方；若收到的是便携授权则无需提交机器码。
3. 收到供应方提供的 `.lic` 文件后点击【导入授权文件】，也可以直接粘贴注册码。
4. 点击【注册授权】。
5. 主界面显示“授权正常”后即可启动桥接。

供应方签发可与 EXE 一起发送的便携授权：

```bat
python tools\generate_license.py --owner "客户名称" --portable --expires 2099-12-31 --output 客户名称.lic
```
...
`license_private/license_private_key.json` 是唯一签发私钥，不进入 EXE，也不能发送给客户。请离线备份；丢失私钥后无法继续签发与当前上位机公钥匹配的新授权。重新生成密钥会使已签发授权失效。
